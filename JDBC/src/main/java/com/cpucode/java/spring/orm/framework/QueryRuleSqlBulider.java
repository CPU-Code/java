...
*/
    private void processLike(QueryRule.Rule rule) {
        ...
        dd(rule.getAndOr(),rule.getPropertyName(),"like","%"+rule.getValues()[0]+"%");
    } /
            **
            * 处理 between
* @param rule
*/
    private void processBetween(QueryRule.Rule rule) {
        ...
        dd(rule.getAndOr(),rule.getPropertyName(),"","between",rule.getValues()[0],"and");
    ...
    private void processEqual(QueryRule.Rule rule) {
        ...
        dd(rule.getAndOr(),rule.getPropertyName(),"=",rule.getValues()[0]);
    } /
            **
            * 处理 <>
* @param rule
...
*/
    private void inAndNotIn(QueryRule.Rule rule,String name){
        ...
        f ((rule.getValues().length == 1) && (rule.getValues()[0] != null)
            ...
            for (int i = 0; i < list.length; i++) {
                ...
                f(i > 0 && i < list.length - 1){
                    add(0,"",",","",list[i],"");
                } i
                f(i == list.length - 1 && i != 0){
                    add(0,"",",","",list[i],")");
                }
            }
        }
...
private void appendWhereSql(){
        ...
        his.whereSql = removeSelect(removeOrders(whereSql.toString()));
        }
